...
public class HDbase<T> extends AbstractCollection<T> {
	...
	Hashtable<T, Integer> heavyBagTable;
	// just created a hashtable for the bag

	
	public HDbase() {
		heavyBagTable = new Hashtable<T, Integer>();
	...
	public boolean add(T o) {
		if (heavyBagTable.containsKey(o)) {
			// Element already exists, increment its count
			heavyBagTable.put(o, heavyBagTable.get(o) + 1);
		} else {
			// Element not in the bag, add it with a count of 1
			heavyBagTable.put(o, 1);
		}
		return true;
		// always returns true
	}
	...
	public boolean addMany(T o, int count) {
		...
		if (heavyBagTable.containsKey(o)) {
			heavyBagTable.put(o, heavyBagTable.get(o) + count);
			// if the key already exists add on to the existing element
		} else {
			heavyBagTable.put(o, count);
	...
	public String toString() {
		String answer = "";
		for (T elements : heavyBagTable.keySet()) {
	...
	public boolean equals(Object o) { // works
		...
		if (this.heavyBagTable.size() != other.heavyBagTable.size()) {
			return false;
			// Check if the heavybags have the same size
		}
		for (T element : heavyBagTable.keySet()) {
	...
	public int hashCode() {
		...
		for (T elements : heavyBagTable.keySet()) {
			if (elements == null) {
				...
				int hash = (elements.hashCode() + heavyBagTable.get(elements));
				// getting hashcode
	...
	public Iterator<T> iterator() {
		return new Iterator<T>() {
			private Iterator<T> keyIterator = heavyBagTable.keySet().iterator();
			...
			public T next() {
				if (currentCount == 0) {
					...
					currentCount = heavyBagTable.get(currentKey);
			...
			public void remove() {
				if (contains(currentKey)) {
					int countInBag = heavyBagTable.get(currentKey);
					if (countInBag > 1) {
						// if count in the bag is bigger than 1
						heavyBagTable.put(currentKey, countInBag - 1);
	...
	public Set<T> uniqueElements() {
		...
		for (T uniElement : heavyBagTable.keySet()) {
	...
	public int getCount(Object o) {
		...
		Integer count = heavyBagTable.get(o);
		// getting count
		if (count == null) {
			return 0;
	...
	public T choose(Random r) {
		...
		for (T element : heavyBagTable.keySet()) {
			int count = heavyBagTable.get(element);
			// getting count
	...
	public boolean remove(Object o) { // may not work , but it works
		if (heavyBagTable.containsKey(o)) {
			int count = heavyBagTable.get(o);
			if (count > 1) {
				heavyBagTable.put((T) o, count - 1);
				// unchecked cast
				// putting the element and the count minus 1

			} else {
				heavyBagTable.remove(o);
	...
	public int size() {
		int totalSize = 0;
		for (Integer size : heavyBagTable.values()) {
			totalSize += size;
